...
const AnalyticsFilters = ({ filterRange, onFilterRangeChange, customRange, onCustomRangeChange }) => {
  ...
  return (
    <div className="flex flex-col gap-4 glass-card p-4 rounded-2xl border border-slate-200/40 dark:border-white/5 shadow-sm transition-all duration-200">
      <div className="flex flex-wrap items-center justify-between gap-3">
        {/* Preset Selector Buttons */}
        <div className="flex flex-wrap items-center gap-1.5 bg-slate-100/40 dark:bg-slate-950/20 p-1 rounded-xl border border-slate-200/60 dark:border-white/5">
          {presets.map((preset) => (
            <button
              key={preset}
              type="button"
              ...
              className={`px-4 py-2 text-xs font-bold rounded-xl cursor-pointer transition-all duration-150 active:scale-95 ${
                filterRange === preset
                  ? 'bg-white/90 dark:bg-white/10 text-primary dark:text-white shadow-xs border border-slate-200/40 dark:border-white/5 font-extrabold'
      ...
      {filterRange === 'Custom Range' && (
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-3 border-t border-slate-200/40 dark:border-white/5 animate-fade-in">
          <div className="flex-1 flex flex-col gap-1">
            <label htmlFor="start-date" className="text-[10px] font-bold tracking-wider text-slate-450 dark:text-slate-400 uppercase">
              Start Date
            </label>
            <input
              id="start-date"
